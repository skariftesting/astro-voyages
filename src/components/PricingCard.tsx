...
const PricingCard = ({
  ...
  return (
    <div
      className={`glass-panel relative ${popular ? 'border-space-cyan/40 ring-2 ring-space-cyan/20' : ''} ${className} transition-all duration-500`}
    >
      {popular && (
      ...
      <div className="p-6 relative z-10 group">
        ...
        <p className="mt-2 text-space-gray text-sm group-hover:text-space-light-gray transition-colors duration-300">{description}</p>
        ...
        <div className="space-y-3">
          {features.map((feature, index) => (
            <div key={index} className="flex items-start group/feature hover:translate-x-1 transition-transform duration-300">
              ...
              <span className={`text-sm transition-colors duration-300 ${feature.included ? 'text-space-white group-hover/feature:text-space-cyan' : 'text-space-gray line-through'}`}>
                {feature.title}
              </span>
            </div>
          ))}
            ...
            <Button 
              className={`w-full transition-all duration-300 hover:shadow-lg relative z-10 ${
                popular 
                  ...
                  : 'bg-space-light-blue/50 text-space-white hover:bg-space-light-blue/70 hover:shadow-space-light-blue/20 hover:scale-105'
              }`}
            >
              Select Package
            </Button>
